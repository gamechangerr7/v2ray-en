...
if [[ -f /etc/redhat-release ]]; then
    ...
    echo -e "${red}Unable to detect the system version. Please contact the script author!${plain}\n" && exit 1
fi
...
if [[ $arch == "x86_64" || $arch == "x64" || $arch == "amd64" ]]; then
    ...
    arch="amd64"
    echo -e "${red}Failed to detect architecture. Using the default architecture: ${arch}${plain}"
fi
...
config_after_install() {
    echo -e "${yellow}For security reasons, after installation/update, you need to change the port and account password forcefully.${plain}"
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        ...
        echo -e "${yellow}Confirming settings, configuring...${plain}"
        /usr/local/x-ui/x-ui setting -username ${config_account} -password ${config_password}
        echo -e "${yellow}Account password set completed.${plain}"
        /usr/local/x-ui/x-ui setting -port ${config_port}
        echo -e "${yellow}Panel port set completed.${plain}"
    else
        echo -e "${red}Cancelled. All settings are set to default. Please change them promptly.${plain}"
    fi
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}Failed to check x-ui version. It may exceed the Github API limit. Please try again later or specify the x-ui version for installation manually.${plain}"
            exit 1
        fi
        echo -e "Detected the latest version of x-ui: ${last_version}. Starting installation"
    ...
    echo -e "${green}x-ui v${last_version}${plain} installation completed. The panel has been started."
    echo -e ""
    echo -e "Usage of x-ui management script:"
    echo -e "----------------------------------------------"
    echo -e "x-ui              - Display management menu (with more features)"
    ...
    echo -e "x-ui v2-ui        - Migrate v2-ui account data to x-ui on this machine"
